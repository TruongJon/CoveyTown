  ...
  registerSocketListeners() {
    ...
    this._socket.on('interactableUpdate', interactable => {
      if (isConversationArea(interactable)) {
        const conversationArea = this.conversationAreas.find(c => c.id === interactable.id);
        if (conversationArea) {
          const emptyBefore = conversationArea.isEmpty();
          conversationArea.topic = interactable.topic;
          conversationArea.occupants = this._playersByIDs(interactable.occupantsByID);
          if (emptyBefore !== conversationArea.isEmpty()) {
        ...
        const updatedPosterSessionArea = this._posterSessionAreas.find(
          eachArea => eachArea.id === interactable.id,
        );
        updatedPosterSessionArea?.updateFrom(interactable);
      }
    });
  }
